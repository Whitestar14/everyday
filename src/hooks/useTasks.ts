...
export function useTasks() {
    ...
    // UI-level add: rely on store pipeline (no parsing here)
    const addTask = (text: string, type: 'task' | 'routine' = 'task', metadata?: Partial<Task>) => {
        try {
            TaskService.validateTask(text.trim());
            storeAddTask(text, type, metadata);
            return true;
        } catch (error) {
