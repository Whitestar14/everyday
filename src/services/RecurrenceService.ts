...
function ensureDtstart(opts: Partial<RRuleOptions>, extra?: { dtstart?: Date }): void {
  if (!opts.dtstart) {
    opts.dtstart = extra?.dtstart ?? new Date()
  }
}
...
export function createRRule(
  ...
  // Base frequencies
  if (/\b(daily|every\s+(?:other|\d+)\s+day|every day)\b/.test(lower)) {
    opts.freq = RRuleClass.DAILY
    opts.interval = parseInterval(lower)
  ...
  } else if (/\byearly\b/.test(lower) || /\bannually\b/.test(lower) || /\bevery\s+year\b/.test(lower)) {
  ...
  // Specific weekdays list or single weekday
  const listMatch = lower.match(
    /\bevery\s+((?:mon|tue|wed|thu|fri|sat|sun|monday|tuesday|wednesday|thursday|friday|saturday|sunday)(?:[,\s]+(?:mon|tue|wed|thu|fri|sat|sun|monday|tuesday|wednesday|thursday|friday|saturday|sunday))*)\b/
  )
  if (listMatch) {
    ...
    const singleDayMatch = lower.match(
      /\bevery\s+(mon|tue|wed|thu|fri|sat|sun|monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/
    )
    if (singleDayMatch) {
  ...
  if (/\bevery\s+other\s+week\b/.test(lower)) {
    ...
    const dmatch = lower.match(
      /\bon\s+(mon|tue|wed|thu|fri|sat|sun|monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/
    )
    if (dmatch) {
  ...
  // Apply extras and ensure we have a dtstart anchor by default
  ensureDtstart(opts, extra)
...
export function getNextOccurrence(rule: RRule, after?: Date, inclusive: boolean = false): Date | null {
  const base = after ?? new Date()
  return rule.after(base, inclusive) ?? null
}
