...
vi.mock("@/services/NotificationService", () => ({
  ...
  cancelTaskNotifications: vi.fn().mockResolvedValue(undefined),
}))
...
describe("Task Store", () => {
  ...
  it("updates task metadata and reschedules notifications", async () => {
    useTaskStore.getState().addTask("Task with due date", "task", {
      dueDate: new Date(),
    })
    const id = useTaskStore.getState().tasks[0].id
    await useTaskStore.getState().updateTaskMetadata(id, { dueDate: new Date() })
    expect(useTaskStore.getState().tasks[0].dueDate).toBeInstanceOf(Date)
  })

  it("completes a normal task and removes it after undo window", async () => {
